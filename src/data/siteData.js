// Data untuk section Tentang Dusun
export const tentangDusun = [
  {
    title: 'Lokasi Dusun',
    description:
      'Dusun Ngalang berada di wilayah pedesaan yang masih asri dan mudah dikenali melalui peta titik Balai Desa Ngalang.',
  },
  {
    title: 'Lingkungan Alam',
    description:
      'Wilayahnya dikelilingi perbukitan karst khas Gunungkidul dan lahan pertanian yang subur, sehingga suasananya tenang dan natural.',
  },
  {
    title: 'Mata Pencaharian',
    description:
      'Sebagian besar penduduk Padukuhan Jambu bermata pencaharian sebagai petani, peternak, dan pelaku usaha kecil atau UMKM.',
  },
  {
    title: 'Budaya & Tradisi',
    description:
      'Masyarakat juga mengembangkan wisata lokal berbasis alam dan budaya dengan semangat gotong royong yang tinggi, termasuk tradisi tahunan Nyadran Gunung Genthong dan Rasul Gubug Gedhe sebagai wujud syukur pascapanen.',
  },
]

// Data untuk section UMKM
export const daftarUmkm = [
  {
    name: 'Warung Makan Bu Siti',
    category: 'Kuliner',
    image:
      'https://images.unsplash.com/photo-1499529112087-3cb3b73cec95?auto=format&fit=crop&w=1200&q=80',
    description:
      'Menyediakan berbagai menu masakan tradisional Jawa yang lezat dan terjangkau.',
    address: 'Jl. Raya Ngalang No. 12',
  },
  {
    name: 'Kerajinan Bambu Pak Hadi',
    category: 'Kerajinan',
    image:
      'https://images.unsplash.com/photo-1596704017254-9d31f0c8f0d8?auto=format&fit=crop&w=1200&q=80',
    description:
      'Produsen dan penjual berbagai kerajinan bambu berkualitas tinggi untuk kebutuhan rumah tangga.',
    address: 'Jl. Ngalang Timur No. 5',
  },
  {
    name: 'Batik Ngalang',
    category: 'Fashion',
    image:
      'https://images.unsplash.com/photo-1618316522540-6ca0f9c1d892?auto=format&fit=crop&w=1200&q=80',
    description:
      'Toko batik dengan motif khas Ngalang, tersedia batik tulis dan batik cap untuk souvenir.',
    address: 'Jl. Pasar Ngalang No. 8',
  },
]

// Data untuk section Acara
const kegiatanRasulan = [
  'Tasyakuran dan doa bersama sebagai ungkapan syukur atas panen.',
  'Penyajian tumpeng serta aneka makanan tradisional hasil olahan warga.',
  'Pertunjukan kesenian tradisional yang melibatkan kelompok pemuda dan tokoh budaya.',
  'Makan bersama seluruh warga untuk memperkuat kebersamaan lintas generasi.',
  'Gotong royong menyiapkan rangkaian acara dari tahap perencanaan hingga pelaksanaan.',
]

export const daftarAcara = [
  {
    kategori: 'Tradisi',
    judul: 'Rasulan Dusun Ngalang',
    tanggal: '15 Juni 2026',
    lokasi: 'Balai Dusun',
    deskripsi:
      'Upacara adat tahunan Nyadran Gunung Genthong dan Rasul Gubug Gedhe sebagai wujud syukur pascapanen yang mempertemukan seluruh warga dusun.',
    kegiatan: kegiatanRasulan,
  },
  {
    kategori: 'Sosial',
    judul: 'Gotong Royong Lingkungan',
    tanggal: '29 Juni 2026',
    lokasi: 'Seluruh Dusun',
    deskripsi:
      'Kegiatan bersih-bersih dan perbaikan fasilitas umum dusun yang melibatkan seluruh warga lintas RT sebagai wujud semangat kebersamaan.',
    kegiatan: [
      'Bersih-bersih jalan dan saluran air antar-RT.',
      'Pengecatan dan perbaikan fasilitas balai dusun.',
      'Penanaman pohon di area publik dan pinggir jalan.',
      'Penyediaan konsumsi bersama oleh ibu-ibu PKK.',
      'Evaluasi dan musyawarah program kerja dusun.',
    ],
  },
  {
    kategori: 'Nasional',
    judul: 'Peringatan HUT RI ke-81',
    tanggal: '17 Agustus 2026',
    lokasi: 'Lapangan Dusun',
    deskripsi:
      'Perayaan Hari Kemerdekaan Republik Indonesia dengan upacara bendera, lomba tradisional, dan pentas seni warga yang meriah dan penuh semangat.',
    kegiatan: [
      'Upacara bendera khidmat bersama warga dan perangkat dusun.',
      'Lomba panjat pinang, balap karung, dan tarik tambang.',
      'Lomba memasak dan kreasi jajan pasar ibu-ibu PKK.',
      'Pentas seni dan pertunjukan budaya malam hari.',
      'Pemberian penghargaan bagi warga berprestasi dusun.',
    ],
  },
]

// Data untuk section Berita
export const daftarBerita = [
  {
    title: 'Persiapan Rasulan 2026 Mulai Dilakukan Warga',
    date: '05 Juni 2026',
    kategori: 'Kegiatan',
    penulis: 'Tim Dusun Ngalang',
    image:
      'https://images.unsplash.com/photo-1464207687429-7505649dae38?auto=format&fit=crop&w=900&q=80',
    excerpt:
      'Warga Dusun Ngalang memulai kerja bakti lintas RT untuk menyiapkan lokasi, konsumsi, serta rangkaian kegiatan budaya.',
  },
  {
    title: 'Pelatihan Branding Produk UMKM Desa',
    date: '29 Mei 2026',
    kategori: 'UMKM',
    penulis: 'Tim Dusun Ngalang',
    image:
      'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&w=900&q=80',
    excerpt:
      'Pendampingan pemasaran digital diikuti pelaku usaha lokal agar produk Dusun Ngalang lebih dikenal luas.',
  },
  {
    title: 'Jalur Kunjungan ke Balai Desa Diperbarui',
    date: '18 Mei 2026',
    kategori: 'Pengumuman',
    penulis: 'Tim Dusun Ngalang',
    image:
      'https://images.unsplash.com/photo-1576076983530-d45f9c5b60b2?auto=format&fit=crop&w=900&q=80',
    excerpt:
      'Pemerintah desa memperbarui informasi rute dan titik parkir untuk memudahkan tamu yang berkunjung.',
  },
]

// Data untuk section Galeri
export const daftarGaleri = [
  {
    image:
      'https://images.unsplash.com/photo-1523741543316-beb7fc7023d8?auto=format&fit=crop&w=1200&q=80',
    caption: 'Hamparan sawah saat musim tanam',
    category: 'Alam',
  },
  {
    image:
      'https://images.unsplash.com/photo-1473448912268-2022ce9509d8?auto=format&fit=crop&w=1200&q=80',
    caption: 'Perbukitan karst di sekitar Ngalang',
    category: 'Alam',
  },
  {
    image:
      'https://images.unsplash.com/photo-1499529112087-3cb3b73cec95?auto=format&fit=crop&w=1200&q=80',
    caption: 'Ladang gandum keemasan di pinggiran dusun',
    category: 'Alam',
  },
  {
    image:
      'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&w=1200&q=80',
    caption: 'Produk UMKM unggulan dusun',
    category: 'UMKM',
  },
  {
    image:
      'https://images.unsplash.com/photo-1464226184884-fa280b87c399?auto=format&fit=crop&w=1200&q=80',
    caption: 'Hasil panen sayuran segar warga dusun',
    category: 'Alam',
  },
  {
    image:
      'https://images.unsplash.com/photo-1464207687429-7505649dae38?auto=format&fit=crop&w=1200&q=80',
    caption: 'Suasana gotong royong masyarakat',
    category: 'Kegiatan',
  },
]

// Data untuk section Lokasi
export const dataGeografis = [
  {
    label: 'Wilayah Administratif',
    value: 'Kec. Gedangsari, Kab. Gunungkidul, D.I. Yogyakarta',
  },
  {
    label: 'Titik Referensi Utama',
    value: 'Balai Desa Ngalang (pusat layanan dan orientasi kunjungan)',
  },
  {
    label: 'Karakter Bentang Alam',
    value: 'Perbukitan, lahan pertanian, dan permukiman warga berbasis komunitas',
  },
  {
    label: 'Akses Kunjungan',
    value: 'Dapat dijangkau kendaraan roda dua/empat melalui jalur utama Gedangsari',
  },
]
